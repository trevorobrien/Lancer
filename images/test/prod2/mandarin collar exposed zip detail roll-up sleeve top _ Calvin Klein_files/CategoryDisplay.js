...
categoryDisplayJS={
	...
	/** Array of EntitledItems used in Bundles **/
	entitledItemsArray : [],
	...
	//WARNING: This should never be set/used on a Akamai cached page (DPC=Yes)
	...
	**/
	...
	/**
	 * variable for the Tax calculation.
	 */
	updateTaxes : false,
	...
	/**
	 * This array holds the json object returned from the service, holding the
	 * price information of the catEntry.
	 */
	itemInventoryJsonOject : [],
	...
	**/
	setCommonParameters:function(langId,storeId,catalogId,userType){
		...
		this.userType = userType; //WARNING: This should never be set/used on a Akamai cached page (DPC=Yes)
	},
	
	/**
	 * setUpdateTaxes This function will set the flag "updateTaxes" which is
	 * used to determine if the shopping cart needs taxes to be updated.
	 * 
	 * @param {Boolean}
	 *            updateTaxes Flag which indicates whther to use taxes will be
	 *            updated.
	 * 
	 */
	setUpdateTaxes : function(updateTaxes) {
		this.updateTaxes = updateTaxes;
	},
	...
	**/
	...
	/**
	 * setEntitledItemsArray Sets an array of entitled items for a bundle with
	 * multiple products. This function is used in CachedBundleDisplay.jsp to
	 * add all the entitled SKUs of the products in a particular bundle to this
	 * array. The array that is generated is used later in
	 * {@link fastFinderJS.resolveSKU}.
	 * 
	 * @param {Object}
	 *            entitledItemArray An object which holds both the catalog entry
	 *            ID as well as an array of attributes for the entitled items of
	 *            a product.
	 * 
	 */
	...
	**/
	changePrice : function(entitledItemId,isPopup,displayPriceRange){
		...
		if(catalogEntryId!=null){
			...
			if(this.itemPriceJsonOject[catalogEntryId] != null && this.itemPriceJsonOject[catalogEntryId] != 'undefined'){
				...
				this.updateQuantity(this.itemPriceJsonOject[catalogEntryId].catalogEntry);
			...
			//if json object is not present, call the service to get the details.
				...
				parameters.catalogEntryId = catalogEntryId; // this variable is used in GetCatalogEntryDetailsByIDView
	...
	/**
	 * Updates the quantity drop down with the JSON object.
	 * 
	 * @param {object}
	 *            catEntry The JSON object with catalog entry details.
	 */
	updateQuantity : function(catEntry) {
		console.log(catEntry);
		// check if the catEntry has preorder attribute set to true.
		var isPreOrder;
		for (var i=0;i<catEntry.catalogEntryAttributes.length;i++) {
			if (catEntry.catalogEntryAttributes[i].usage == "Descriptive" && catEntry.catalogEntryAttributes[i].name == "PreOrder" && catEntry.catalogEntryAttributes[i].values[0].value == '1' ) {
				isPreOrder=true;
			}
		}
		var $inventoryMessage=$('.inventoryStatus');
		$inventoryMessage.removeClass('high low out');
		var qty = parseInt(catEntry.inventory.quantity);
		if(isPreOrder)
		{
			$("#product .preorderHelpContainer").removeClass(U.HIDDEN);
			//RJUN-8 CK - Size-Level Pre-Order Display
			if (catEntry.startDate !=undefined && catEntry.startDate !="")
			{
				$("#product .preOrderSku").removeClass(U.HIDDEN);
				$("#product .preOrderSku .preOrderexpdate").text(catEntry.startDate);
				$("#product #add2CartBtn .preorder").removeClass(U.HIDDEN);
				$("#product #add2CartBtn .addtocart").addClass(U.HIDDEN);
			}else{
				$("#product .preOrderSku").addClass(U.HIDDEN);
				$("#product #add2CartBtn .addtocart").removeClass(U.HIDDEN);
				$("#product #add2CartBtn .preorder").addClass(U.HIDDEN);
				// RJUN-100 SP/TH: The SKU which are made pre order are not reflected in DR stage and prod environments
				//$("#product .preorderHelpContainer").addClass(U.HIDDEN);
			}
			$inventoryMessage.text(U.EMPTY);
			if(qty<3)
			{
				if(qty==0)$inventoryMessage.addClass('out');
				else if(qty==1) $inventoryMessage.addClass('low');
				else $inventoryMessage.addClass('low');
			} else $inventoryMessage.addClass('high');
		} else
		{
			$("#product .preorderHelpContainer").addClass(U.HIDDEN);
			$("#product .preOrderSku").addClass(U.HIDDEN);
			...
			if(qty<3)
			{
				if(qty==0)$inventoryMessage.text(MessageHelper.messages["INV_SIZE_ZERO_STOCK"]).addClass('out');
				else if(qty==1) $inventoryMessage.text(MessageHelper.messages["INV_SIZE_ONE_STOCK"]).addClass('low');
				else $inventoryMessage.text(MessageHelper.messages["INV_SIZE_LOW_STOCK"].replace('{0}',qty)).addClass('low');
			} else $inventoryMessage.text(MessageHelper.messages["INV_SIZE_IN_STOCK"]).addClass('high');
		}
		
		U.setupDescription(true);
	 ...
	 */	
	 displayPriceServiceResponse : function(serviceResponse, ioArgs){
		...
		categoryDisplayJS.handleInventoryServiceResponse(serviceResponse);
		categoryDisplayJS.updateQuantity(serviceResponse.catalogEntry);
	 },
	 
	 /**
	 * Updates inventory of the catEntry selected with the JSON object returned
	 * from the server.
	 * 
	 * @param {object}
	 ...
	 */
	 ...
	 handleInventoryServiceResponse : function(serviceResponse) {

			// stores the json object, so that the service is not called when same
			// catEntry is selected.
			this.itemInventoryJsonOject[serviceResponse.catalogEntry.catalogEntryIdentifier.uniqueID] = serviceResponse;
	},
	 ...
	 */	
	 displayPrice : function(catEntry){
		...
		if(popup == false){
			var innerHTML = "";
			if(catEntry.listPriced == 'false'){
				innerHTML = "<span class='price offerPrice'>" + catEntry.offerPrice + "</span>";
			}
			else{
				innerHTML = "<span class='price listPrice'>" + catEntry.listPrice + "</span>"+
							"<span class='price offerPrice'>" + catEntry.offerPrice + "</span>";
			}
			...
			if(categoryDisplayJS.displayPriceRange == true){
				for(var i in catEntry.priceRange){
					if(catEntry.priceRange[i].endingNumberOfUnits != 'null'){
						...
						innerHTML = innerHTML + "<span class='price offerPrice'>" + tempString + "</span>";
					}
					else{
			...
			//TODO
			document.getElementById('price_display').innerHTML = innerHTML;
	...
	**/
	getCatalogEntryId : function(){
		...
		for(attribute in this.selectedAttributes){
			attributeArray.push(attribute + "_|_" + U.addUnderscoreEncoding(this.selectedAttributes[attribute]));
		}
		return this.resolveSKU(attributeArray);
	},
	
	/**
	 * getCatalogEntryIdforBundleProduct Returns the catalog entry ID for a
	 * catalog entry that has the same attribute values as a specified product's
	 * selected attributes as passed in via the selectedAttributes parameter.
	 * 
	 * @param {String[]}
	 *            selectedAttributes The array of selected attributes upon which
	 *            to resolve the SKU.
	 * 
	...
	getCatalogEntryIdforBundleProduct : function(productId, selectedAttributes) {
		...
		for (attribute in selectedAttributes) {
			attributeArray.push(attribute + "_" + U.addUnderscoreEncoding(selectedAttributes[attribute]));
		}
	...
	**/
	getCatalogEntryIdforProduct : function(selectedAttributes){
		var attributeArray = [];
		for(attribute in selectedAttributes){
			attributeArray.push(attribute + "_|_" + U.addUnderscoreEncoding(selectedAttributes[attribute]));
		}
		return this.resolveSKU(attributeArray);
	},
	...
	**/
	...
	/**
	 * resolveSKUForBundleProduct Resolves a SKU using an array of defining
	 * attributes. Function to be used for bundles since there can be multiple
	 * products in the bundle.
	 * 
	 * @param {String}
	 *            productId One of product id from the bundle.
	 * @param {String[]}
	 *            attributeArray An array of defining attributes upon which to
	 *            resolve a SKU.
	 * 
	 * @return {String} catentry_id The catalog entry ID of the SKU.
	 * 
	...
	resolveSKUForBundleProduct : function(productId, attributeArray) {
		...
		if(typeof(this.entitledItemsArray[productId])=='undefined') return null;
		for (var x=0;x<this.entitledItemsArray[productId].length;x++)
		{
			...
			// Handle special case where a catalog entry has one sku with no
			// attributes
			if (attributeArrayCount == 0 && attributeCount == 0) {
				return catentry_id;
			}
			if (attributeCount != 0 && attributeArrayCount >= attributeCount) {
				...
				for (var attributeName=0;attributeName<attributeArray.length;attributeName++) {
					var attributeValue = attributeArray[attributeName];
					if (attributeValue in Attributes) {
						matchedAttributeCount++;
					}
	...
	**/
	 ...
	 */
	 ...
	 /**
	  * AddBundleComponent2ShopCartAjax This function is used to add a bundle component to the shopping cart. 
	  * This is for the ajax flow which will take a form as input and retrieves all the items catentry IDs and adds them to the form.
	  *						 
	  * @param {form} form The form which contains all the inputs for the component.
	  *						The form is expected to have the following values: 
	  *						catEntryId_<index> where index is between 1 and numberOfProduct.
	  **/
	 AddBundleComponent2ShopCartAjax : function(form, index, errorParentId){
			...
			var catEntryId = form["catEntryId_" + index].value;
			this.addedProductId = catEntryId;
			
			var bundleComponentId = catEntryId;
			...
			var $qty = document.getElementById('quantity_' + bundleComponentId);
			var qty = $qty ? $qty.value : null;

			if (catEntryId == null) {
				// size not selected
				var sizeContainer = document.getElementById("sizes_" + index); 
				if(sizeContainer)
					MessageHelper.formErrorHandleClient("sizes_" + index, MessageHelper.messages['ERR_RESOLVING_SKU'], errorParentId);
				else
			...
			if (qty == null || qty <= 0) {
				...
				MessageHelper.formErrorHandleClient("quantity_section_" + bundleComponentId, MessageHelper.messages["BUNDLE_ERR_SELECT_QTY"]);
			...
			// DP:TODO - check if this is needed
			...
			cursor_wait();		
			wc.service.invoke("AddBundleOrderItem", params);
	...
	AddBundle2ShopCartAjax : function(form){
		...
		for(var i = 1; i <= productCount; i++){
			...
			if (catEntryId != null) {
				...
				// size not selected
				var sizeContainer = document.getElementById("sizes_" + i); 
				if(sizeContainer) {
					MessageHelper.formErrorHandleClient("sizes_" + i, MessageHelper.messages['ERR_RESOLVING_SKU']);
					// scroll to the component error message
					U.scrollTo($("#sizes_" + i).offset().top - 100, null, null, null, null);
				}
				else
					MessageHelper.displayErrorMessage(MessageHelper.messages['ERR_RESOLVING_SKU']);
				return;
		...
		if (componentsAdded <= 0) {
			MessageHelper.displayErrorMessage(MessageHelper.messages["BUNDLE_ERR_SELECT_SKU"]);
			return;
		} else {
			// For Handling multiple clicks
			if (!submitRequest()) {
	...
	*
	**/
	ReplaceItemAjax : function(entitledItemId,quantity,replaceOrderItemId){
		...
		if(catalogEntryId!=null){
				if(this.itemInventoryJsonOject[catalogEntryId]!= undefined)
				{
					var inventoryQty = parseInt(this.itemInventoryJsonOject[catalogEntryId].catalogEntry.inventory.quantity);
					var availableQty;
					
					//check for exiting quantity from orderitem,if the edited sku is not the same as the original opened one
					if(this.itemInventoryJsonOject[catalogEntryId].catalogEntry.availableToAdd != undefined)
					{
						availableQty = parseInt(this.itemInventoryJsonOject[catalogEntryId].catalogEntry.availableToAdd.quantity);
					}
					
					if(quantity == null || quantity == "" || parseInt(quantity) < 1){
							MessageHelper.formErrorHandleClient('replaceBtn',MessageHelper.messages['QUANTITY_INPUT_ERROR']);
							return;
					}
					else if( (inventoryQty < 1 ) ||
							 (inventoryQty > 0 && availableQty != undefined && availableQty == 0 && parseInt(quantity) > inventoryQty ) )
					{
						//displaying the error message based on the existing element id.
						if($("#replaceBtn").length)
							MessageHelper.displayErrorMessage(MessageHelper.messages['ITEM_LEVEL_OOS_MSG']);
						else
							MessageHelper.displayErrorMessage(MessageHelper.messages['ITEM_LEVEL_OOS_MSG']);
						return;
					
					} 
					else if(parseInt(quantity) > inventoryQty)
					{
						categoryDisplayJS.actualQty = inventoryQty;
						quantity = inventoryQty;
					}

				}
				var secPopup =dijit.byId('second_level_category_popup');
				if(secPopup != null) {
					secPopup.hide();
				}
			...
			var errorMessage = MessageHelper.messages['ERR_RESOLVING_SKU'];
			MessageHelper.formErrorHandleClient('replaceBtn', errorMessage);
		}
	},
	...
	ReplaceItemAjaxHelper : function(catalogEntryId,qty,removeOrderItemId,addressId,shipModeId){
		...
		wc.service.declare({
			...
			,successHandler: function(serviceResponse) {
				...
				if(!shipInfoUpdateNeeded){
					...
					wc.service.invoke("AjaxAddOrderItem_Replace", params2);
					if(O.isOpen())
						O.close();
				}
				else{
				...
				//WI29424 CK - Infinite loading is happening on clicking update button from edit overlay
				var partNumber=$("#product").data().partNumber;
				PVHTealium.partNumber = partNumber;
		...
		wc.service.declare({
			...
			,successHandler: function(serviceResponse) {
				...
				params3.orderItemId = serviceResponse.orderItemId;
				params3.customParam = "1";
				wc.service.invoke("OrderItemAddressShipMethodUpdate", params3);
			}

			,failureHandler: function(serviceResponse) {
				if(serviceResponse.errorMessageKey == '_ERR_INVENTORY_NOT_AVAILABLE') {
					MessageHelper.displayErrorMessage(MessageHelper.messages["SHOPCART_REPLACED_ITEM"]);
					...
					// check if Gift Box sku present in order, if present and the item deleted is not gift box reduce count by 1
					if(document.getElementById("giftBoxSkuId")) {
						if(removeOrderItemId != document.getElementById("giftBoxSkuId").value) 
							y = y - 1;
					}
					if(y == 1){
						...
						dijit.byId('WC_ShipmentDisplay_div_17').refresh();
						dijit.byId('WC_ShipmentDisplay_div_18').refresh();
		 ...
		 */
			...
			,successHandler: function(serviceResponse) {
				...
				var qtyAdded;
				if(categoryDisplayJS){
					if(categoryDisplayJS.actualQty !== undefined && categoryDisplayJS.actualQty != '') {
						qtyAdded = categoryDisplayJS.actualQty;
						categoryDisplayJS.actualQty = '';
					}
				}
	            ...
	            if(serviceResponse.orderItem != null && serviceResponse.orderItem[0] != null && serviceResponse.orderItem[0].orderItemId != null)
	            	MiniCart.updateMiniCart(serviceResponse, qtyAdded);
	            
				if(U.isMobile()) document.location.href = "AjaxOrderItemDisplayView?storeId=" + serviceResponse.storeId[0] + "&catalogId=" + serviceResponse.catalogId[0] + "&langId=" + serviceResponse.langId[0];
		 ...
		 */
			,failureHandler: function(serviceResponse) {
				...
				if(serviceResponse.errorMessageKey == '_ERR_INVENTORY_NOT_AVAILABLE') {
					...
					PVHMiniCart.ajaxUpdateMiniShoppingCartDisplay("");
				} 
				else{
					if (serviceResponse.errorMessage) {
...
setOrderId : function(orderId)
{
	...
	**/
	...
	ajaxAddAllWishListItem2ShopCart : function() {
		...
		if(count >= 1){
			categoryDisplayJS.AddItem2ShopCartAjax(catEntryIdArray, quantityArray, 1);
		}	
	},
}
