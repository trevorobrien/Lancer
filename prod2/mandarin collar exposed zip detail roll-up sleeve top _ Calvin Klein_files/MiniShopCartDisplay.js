...
var order_updated = {	'AjaxAddOrderItem':'AjaxAddOrderItem',
						...
						'AjaxAddOrderItem_Cart':'AjaxAddOrderItem_Cart',
						'AjaxDeleteOrderItem_cart':'AjaxDeleteOrderItem_cart'
					};
...
/** This variable keeps track of actual quantity added to cart. */
var addedQuantity = '';
...
wc.render.declareRefreshController({
	   ...
	   ,modelChangedHandler: function(message, widget) {
			  ...
			  if(message.actionId in order_updated || message.actionId == 'AjaxDeleteOrderItemForShippingBillingPage'){
					 ...
					 if(addedQuantity != '')
						 param.addedQuantity = addedQuantity;
					 
					 //if requested quantity was not added because of lack of inventory ( available already added to cart) do not refresh mini cart notification
...
function populateProductAddedDropdown(){
		...
		for(productId in productAddedList){
			...
			if(document.getElementById('MiniShopCartAddedProdQty_'+productId) != null && productDetails[3] != null && addedQuantity == '' ){
...
function initiatePayPalCheckout(orderTotal) {
	var params = [];
	...
	wc.service.invoke('AjaxPrepareOrderForPayPal', params);
}
