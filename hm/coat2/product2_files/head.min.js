...
if(c=="PRA1"||c=="PRA3"||c=="PRA4"){setVCParameter(c,a);
var b="";
...
(function(a){a.MultiCountry=function(){function c(g){var i=document.cookie.split(";"),h;
if(i&&i.length){i.some(function(k){k=k.trim();
var j=k.split("=");
if(j[0]==g){h=decodeURIComponent(j[1]);
return true
}})
}return h
}this.getCountryCode=function d(j){var i=c(j),g="";
if(i){var h=/(?:^[a-z]{2}_)?([A-Z]{2})$/.exec(i);
if(h){g=h[1]
}}return g
};
this.getLocale=function f(k,j){var g="";
if(!j){var i=c(k);
if(i&&i.search(/^[a-z]{2}_[A-Z]{2}$/)==0){g=i
}}else{var h=this.getCountryCode(k);
if(h.length){g=j+"_"+h
}}return g
};
this.getLanguage=function e(h){var g=this.getLocale(h);
if(!g||!g.length){return""
}else{return g.substring(0,2)
}};
this.createCookie=function b(m,l,k){var j=new Date(),i=182;
j.setTime(j.getTime()+(i*24*60*60*1000));
var g="expires="+j.toUTCString();
var h=m+"="+l+"_"+k+"; "+g+"; domain=.hm.com; path=/";
return h
}
};
...
}}function utagTrackFormLinkCall(a,c,b){if(typeof(utag)!="undefined"){utag.link({conversion_id:a,conversion_category:c,conversion_step:b});
createUtagCookie(a,c,b)
...
}}function setConversionData(e){var f="";
var a="";
var d="";
var c="REGISTRATION";
var b=jQuery.parseJSON(e);
if(typeof(b)=="string"){b=jQuery.parseJSON(b)
}if(b.conversion_id){a=b.conversion_id
}if(b.conversion_category){f=b.conversion_category
}if(b.conversion_step){d=b.conversion_step;
++d
}utag_data.event_type=c;
utag_data.conversion_id=a;
utag_data.conversion_category=f;
utag_data.conversion_step=d
...
}function utagTealiumTrack(b){if(hm.cookies==undefined){hm.cookies=new hm.Cookies()
}var c=false;
var a=null;
if(hm.cookies!=undefined){a=hm.cookies.readCookie("userCookie")
}if(a!=null){c=checkifLogged(a)
}var d=null;
if(hm.cookies!=undefined){d=hm.cookies.readCookie("utagCookie")
}if(d!=null&&c==true){setConversionData(d);
setCustomerData(a);
...
}}function trackJoinClub(){if(typeof(window.utag)!="undefined"){utag.link({event_type:"CLUB_SIGNUP",event_category:"Club",event_id:"Signup",club_signup:"1"})
...
utagTrackFormView("LOGIN","LOGIN","Login : Header",localeInfo.locale,localeInfo.country)
}function trackSubmitLogin(){utagTrackFormLinkCall("Login : Header","Login",1)
}function trackSubmitJoin(){if($("#chb-signin-rollover-join-offers").is(":checked")){trackSignup()
}if($("#club-membership-popdown").is(":checked")){trackJoinClub()
}utagTrackFormLinkCall("Registration : Header","Registration",1)
...
function subscribeNewsLetter(j,c){trackSignup();
var e="",f="",d="",i="0",b="",h="",g="";
e="subscribe";
if(!c){c=getThankYouPage($("#thankyoupage").val())
}h=$("#enablePersonal").val();
f=$("#txt-email").val();
d=$("#newsletter-i-am-kids").is(":checked");
g=$("#chbNewsLetterTerms").is(":checked");
if(h=="true"){i=$("[name=gender]:checked").val()==undefined?3:$("[name=gender]:checked").val();
b=$("#txt-postal-code").val()
}var a='{"email":"'+f+'","firstName":"","lastName":"","gender":'+i+',"postalCode":"'+b+'","kids":"'+d+'","frequency":"'+g+'","birthYear":"","birthMonth":"","birthDay":""}';
$.ajax({type:"POST",url:j,dataType:"json",contentType:"application/json; charset=utf-8",success:function(l){if(l){var k=l.responseStatusCode;
var n=l.responseAlreadyExists;
var m=l.responseStatusMessage;
if(k=="ok"&&n==true){window.location=(c+"?statusCode=successalreadyExist")
}else{if(k=="ok"&&n==false){window.location=(c+"?statusCode=success")
...
a.initialize=function(){setTimeout(function(){a.trackedElementsInPage=[];
console.log("promotion Tracking started");
var b=Math.floor(Math.random()*10000000);
$('*[data-view-tracking="true"]').each(function(){var h="track_"+ ++b;
var e=$(this);
e.attr("id",h);
...
var g;
if(e.attr("data-newarrivals-first-article")){console.log("new arrival mode ON");
g=e.parent().find(".cta, .cta-overlay")
}else{g=e.find(".cta, .cta-overlay")
}console.log("Checkink if we are in case cta-list outside");
if(e.next().hasClass("cta-list")&&e.next().hasClass("moved")){console.log("we are in case mobile cta list");
g=e.next().find(".cta, .cta-overlay")
...
if(type==priv.eventType.POPUP){var elementsToTrack=$element.find("*[data-tracking-type]");
...
return undefined
}}if(type==priv.eventType.ADD_FAVORITE||type==priv.eventType.REMOVE_FAVORITE){if(priv.colorArticleCode!=""){jsonParams.product_id[0]=priv.colorArticleCode
...
}else{if($element.data("tracking-method")&&$element.data("tracking-method")=="link"){if(priv.debugTracking){console.log("viewtrack utag.link will be sent with payload:");
console.log(jsonParams)
}window.utag.link(jsonParams)
...
priv.manageTrackableEvent=function(objectToTrack){var $element=$(objectToTrack.el);
