...
hmApp.controller('HmAppController', ['$scope', 'hmClubService', 'hmCommon',
 	function($scope, hmClubService, hmCommon) {
		...
		//some variable has to be set for authoring mode
		$scope.forceDisplay;
		
		//[PS] PRESHOPPING check if preshopping teasers exists. In that case we hide it by defualt.
		// we are doing it here because AEM side it's difficult to manage it.
		var $teasersPreshopping = $('*[data-preshopping-teaser=true]');
		if($teasersPreshopping){
			console.log("club teaser found: hiding it!");
			var $teaserPreshopping = $($teasersPreshopping[0]);
			$teaserPreshopping.hide();
		}
